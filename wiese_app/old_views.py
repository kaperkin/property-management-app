...
def welcome(request):
    template = loader.get_template('wiese_app/old_welcome.html')
...
def index(request):
    ...
    template = loader.get_template('wiese_app/old_index.html')
    context = RequestContext(request, {
...
def renter_view(request):
    template = loader.get_template('wiese_app/old_renter_view.html')
    context = RequestContext(request)
    return HttpResponse(template.render(context))

def maintenance_all(request):
    maintenance_list = Maintenance.objects.all()
    template = loader.get_template('wiese_app/old_maintenance_all.html')
    context = RequestContext(request,{
        'maintenance_list' : maintenance_list
    })
    return HttpResponse(template.render(context))
...
def detail(request, id):
    if id == "0":
        return HttpResponseRedirect('/old_update_request/0/')
    maintenance = Maintenance.objects.filter(id = id)[0]
    template = loader.get_template('wiese_app/old_detail.html')
    context = RequestContext(request, {
...
def building_spec(request, id):
    if id == "0":
        return HttpResponseRedirect('/old_update_building/0/')
    rental = Rentals.objects.filter(id = id)[0]
    template = loader.get_template('wiese_app/old_building_spec.html')
    context = RequestContext(request, {
        'rental' : rental,
    })
    return HttpResponse(template.render(context))
...
def update_request(request, id):
    ...
    template = loader.get_template('wiese_app/old_update_request.html')
    context = RequestContext(request, {
        'maintenance' : maintenance,
        'rental_list' : rental_list,
    })
...
def update_building(request, id):
    ...
    if request.POST:
        ...
        if request.POST['action'] == 'DELETE':
            ...
            rental.save()
    template = loader.get_template('wiese_app/old_update_building.html')
    context = RequestContext(request, {
        'rental' : rental,
    })
    return HttpResponse(template.render(context))
